...
如果你的程序大部分只会涉及到I/O，比如网络交互，那么使用多线程就很合适， 因为它们
大部分时间都在等待。实际上，你完全可以放心的创建几千个Python线程， 现代操作系统
...
# 在 Normal 的时候, 我们这个 list 扩展4倍, 在 threading 的时候, 
# 我们建立4个线程, 并对运行时间进行对比.
...
# 如果你成功运行整套程序, 你大概会有这样的输出. 
# 我们的运算结果没错, 所以程序 threading 和 Normal 运行了一样多次的运算. 
# 但是我们发现 threading 却没有快多少, 按理来说, 我们预期会要快3-4倍, 因为有建立4个线程, 
